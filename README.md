...
- **pre-commit** (rápido, a cada commit): limpeza de arquivos (espaços em branco, EOF, conflitos de merge), Ruff (lint + format do backend) e ESLint + Prettier (frontend). Corrige automaticamente o que for possível — se algum arquivo for modificado, é só rodar `git add` de novo e commitar.
